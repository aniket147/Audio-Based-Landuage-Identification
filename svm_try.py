...
file_name = 'bio_data.csv'
model_filename = 'model3'
...
y_train_new = [str(x[0]) + str(x[1]) + str(x[2]) + str(x[3]) for x in y_train]
y_valid_new = [str(x[0]) + str(x[1]) + str(x[2]) + str(x[3]) for x in y_validation]
...
svm_model_linear.fit(X_train, y_train_new)
